...
| Artist | Studio | `/studio`, `/studio/branding`, `/studio/stats`, `/studio/governance`, `/studio/updates`, `/studio/audience`, `/studio/stripe`, `/studio/releases`, `/studio/editor` | Stripe submenu only when `stripeConfigured`. Does **not** cover `/library*` (see Library row above). |
