...
- [ ] **(Later) Status bar: icons instead of text, cloud storage total, desktop local-track count/size** — `ConnectedStatusBar.tsx`'s `StatusBarContent` (sounds/notifications/messages/encoding entries are all plain text, e.g. "♪ N sounds", "N unread messages") should use icons instead of text labels. Add a cloud-storage-used entry (total space consumed by the user's uploads). In the desktop app, also show local track count/size alongside cloud storage.
