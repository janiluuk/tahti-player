...
- [ ] **Broadcast dialog: booking calendar link + move Stream Manager in** — see INDEX (`broadcast-dialog-booking-link-and-stream-manager`).
...
- [ ] **Purchase-tier artist editor** — one-time track purchases (`PurchaseTier`) have no artist-facing UI at all (create/edit tier, assign to a track) — found while shipping the PWYW buyer-side amount dialog (`../tahti-org` PR #461 + `TrackDetailView.tsx`, done but not practically reachable without this). See `purchase-tier-artist-editor-missing.md`.
