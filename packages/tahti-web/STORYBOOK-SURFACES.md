# Storybook surface cheat sheet

Lookup before inventing UI. Run `pnpm storybook`. Stories under `packages/storybook/src/`.

Generated from story `title` fields. Prefer matching title; keep live data when swapping.

| Story title | File |
| --- | --- |
| `Tahti/Studio/AddToMusicActions` | `tahti-web/AddToMusicActions.stories.tsx` |
| `Tahti/Media/AddToPlaylistButton` | `tahti-web/AddToPlaylistButton.stories.tsx` |
| `Tahti/Media/AddToPlaylistPanel` | `tahti-web/AddToPlaylistPanel.stories.tsx` |
| `Tahti/Admin/AdminActivityView` | `tahti-web/AdminActivityView.stories.tsx` |
| `Tahti/Admin/AdminAgmView` | `tahti-web/AdminAgmView.stories.tsx` |
| `Tahti/Admin/AdminAnnouncementsView` | `tahti-web/AdminAnnouncementsView.stories.tsx` |
| `Tahti/Admin/AdminDashboardView` | `tahti-web/AdminDashboardView.stories.tsx` |
| `Tahti/Admin/AdminDiscoWidgetsView` | `tahti-web/AdminDiscoWidgetsView.stories.tsx` |
| `Tahti/Admin/AdminFinancialView` | `tahti-web/AdminFinancialView.stories.tsx` |
| `Tahti/Admin/AdminGate` | `tahti-web/AdminGate.stories.tsx` |
| `Tahti/Admin/AdminGovernanceView` | `tahti-web/AdminGovernanceView.stories.tsx` |
| `Tahti/Admin/AdminGrantsView` | `tahti-web/AdminGrantsView.stories.tsx` |
| `Tahti/Admin/AdminI18nView` | `tahti-web/AdminI18nView.stories.tsx` |
| `Tahti/Admin/AdminLogsView` | `tahti-web/AdminLogsView.stories.tsx` |
| `Tahti/Admin/AdminMissedShowsView` | `tahti-web/AdminMissedShowsView.stories.tsx` |
| `Tahti/Admin/AdminModerationView` | `tahti-web/AdminModerationView.stories.tsx` |
| `Tahti/Admin/AdminNav` | `tahti-web/AdminNav.stories.tsx` |
| `Tahti/Admin/AdminNewsView` | `tahti-web/AdminNewsView.stories.tsx` |
| `Tahti/Admin/AdminRadioView` | `tahti-web/AdminRadioView.stories.tsx` |
| `Tahti/Admin/AdminStatusView` | `tahti-web/AdminStatusView.stories.tsx` |
| `Tahti/Admin/AdminStorageUserView` | `tahti-web/AdminStorageUserView.stories.tsx` |
| `Tahti/Admin/AdminStorageView` | `tahti-web/AdminStorageView.stories.tsx` |
| `Tahti/Admin/AdminStreamManagerPanel` | `tahti-web/AdminStreamManagerPanel.stories.tsx` |
| `Tahti/Admin/AdminStreamsView` | `tahti-web/AdminStreamsView.stories.tsx` |
| `Tahti/Admin/AdminTopListsView` | `tahti-web/AdminTopListsView.stories.tsx` |
| `Tahti/Admin/AdminUsersView` | `tahti-web/AdminUsersView.stories.tsx` |
| `Tahti/Admin/AdminVendorsView` | `tahti-web/AdminVendorsView.stories.tsx` |
| `Tahti/Chrome/AppShell` | `tahti-web/AppShell.stories.tsx` |
| `Tahti/Chrome/AppTopNav` | `tahti-web/AppTopNav.stories.tsx` |
| `Studio session` | `tahti-web/ArtistGalleryPanel.stories.tsx` |
| `Tahti/Studio/AudienceVisibilitySection` | `tahti-web/AudienceVisibilitySection.stories.tsx` |
| `Northern Lights — Live` | `tahti-web/AudioEngine.stories.tsx` |
| `Tahti/Studio/AudioRevisionList` | `tahti-web/AudioRevisionList.stories.tsx` |
| `Tahti/Auth/AuthDialog` | `tahti-web/AuthDialog.stories.tsx` |
| `Tahti/Media/BackdropUploadButton` | `tahti-web/BackdropUploadButton.stories.tsx` |
| `Tahti/Studio/BroadcastPreflightPanel` | `tahti-web/BroadcastPreflightPanel.stories.tsx` |
| `Tahti/Channel/ChannelBackdropCard` | `tahti-web/ChannelBackdropCard.stories.tsx` |
| `Tahti/Channel/ChannelChatPanel` | `tahti-web/ChannelChatPanel.stories.tsx` |
| `Tahti/Artist/ChannelControlsWidget` | `tahti-web/ChannelControlsWidget.stories.tsx` |
| `Tahti/Channel/Designer` | `tahti-web/ChannelDesigner.stories.tsx` |
| `Tahti/Channel/Designer/BackdropPanel` | `tahti-web/ChannelDesignerBackdropPanel.stories.tsx` |
| `Tahti/Channel/Designer/ColorSchemeFields` | `tahti-web/ChannelDesignerColorSchemeFields.stories.tsx` |
| `Tahti/Channel/Designer/HeaderStyleTabs` | `tahti-web/ChannelDesignerHeaderStyleTabs.stories.tsx` |
| `Tahti/Channel/Designer/LayoutOnlyLookHint` | `tahti-web/ChannelDesignerLayoutOnlyLookHint.stories.tsx` |
| `Tahti/Channel/Designer/PageBackgroundField` | `tahti-web/ChannelDesignerPageBackgroundField.stories.tsx` |
| `Tahti/Channel/Designer/PlayerGradientControls` | `tahti-web/ChannelDesignerPlayerGradient.stories.tsx` |
| `Tahti/Channel/Designer/PlayerOverlayControls` | `tahti-web/ChannelDesignerPlayerOverlay.stories.tsx` |
| `Tahti/Channel/Designer/PlayerPanel` | `tahti-web/ChannelDesignerPlayerPanel.stories.tsx` |
| `Tahti/Channel/Designer/PlayerVisualizerControls` | `tahti-web/ChannelDesignerPlayerVisualizer.stories.tsx` |
| `Tahti/Channel/Designer/Playlist` | `tahti-web/ChannelDesignerPlaylist.stories.tsx` |
| `Tahti/Channel/Designer/VideoOrImageField` | `tahti-web/ChannelDesignerVideoOrImageField.stories.tsx` |
| `Tahti/Channel/Designer/ElementEditor` | `tahti-web/ChannelElementEditor.stories.tsx` |
| `Tahti/Channel/Designer/LayersMenu` | `tahti-web/ChannelLayersMenu.stories.tsx` |
| `Tahti/Channel/ChannelRadioPlaylistPanel` | `tahti-web/ChannelRadioPlaylistPanel.stories.tsx` |
| `Northern Signals` | `tahti-web/ChannelRotationEditor.latest.stories.tsx` |
| `Tahti/Channel/ChannelTextOverlayEditor` | `tahti-web/ChannelTextOverlayEditor.stories.tsx` |
| `Tahti/Channel/ChannelTextOverlayView` | `tahti-web/ChannelTextOverlayView.stories.tsx` |
| `Tahti/Channel/ChannelView` | `tahti-web/ChannelView.stories.tsx` |
| `Tahti/Channel/ChannelVisualizer` | `tahti-web/ChannelVisualizer.stories.tsx` |
| `Tahti/Widgets/ClientCapabilityNotice` | `tahti-web/ClientCapabilityNotice.stories.tsx` |
| `Tahti/Player/ConnectedPlayerBar` | `tahti-web/ConnectedPlayerBar.stories.tsx` |
| `Tahti/Player/ConnectedQueuePanel` | `tahti-web/ConnectedQueuePanel.stories.tsx` |
| `Tahti/Player/ConnectedSettingsModal` | `tahti-web/ConnectedSettingsModal.stories.tsx` |
| `Tahti/Layout/ConnectedStatusBar` | `tahti-web/ConnectedStatusBar.stories.tsx` |
| `Tahti/Misc/DesktopLibraryPanel` | `tahti-web/DesktopLibraryPanel.stories.tsx` |
| `Tahti/Discover/DirectoryArtistCardGrid` | `tahti-web/DirectoryArtistCardGrid.stories.tsx` |
| `Tahti/Discover/DiscoverGatewayBackground` | `tahti-web/DiscoverGatewayBackground.stories.tsx` |
| `Tahti/Widgets/DiscoWidgetManagerPanel` | `tahti-web/DiscoWidgetManagerPanel.stories.tsx` |
| `Tahti/Widgets/DiscoWidgetsSection` | `tahti-web/DiscoWidgetsSection.stories.tsx` |
| `Tahti/Reference/Element locations` | `tahti-web/ElementLocations.stories.tsx` |
| `Tahti/Media/EmbedButton` | `tahti-web/EmbedButton.stories.tsx` |
| `Tahti/Media/EmbedTrackRow` | `tahti-web/EmbedTrackRow.stories.tsx` |
| `Tahti/Page/EntitySocialHeader` | `tahti-web/EntitySocialHeader.stories.tsx` |
| `Tahti/Studio/FanSubscriptionStats` | `tahti-web/FanSubscriptionStats.stories.tsx` |
| `Tahti/Studio/FanTiersEditor` | `tahti-web/FanTiersEditor.stories.tsx` |
| `Radio Helsinki` | `tahti-web/FavoritesView.stories.tsx` |
| `Aurora (Original Mix)` | `tahti-web/FingerprintTrackPanel.stories.tsx` |
| `Tahti/Studio/FlowGallery` | `tahti-web/FlowGallery.stories.tsx` |
| `Tahti/Player/FullScreenPlayer` | `tahti-web/FullScreenPlayer.stories.tsx` |
| `Tahti/Studio/GenrePicker` | `tahti-web/GenrePicker.stories.tsx` |
| `Tahti/Widgets/GlobalSearch` | `tahti-web/GlobalSearch.stories.tsx` |
| `Tahti/Media/GlowMediaTile` | `tahti-web/GlowMediaTile.stories.tsx` |
| `Tahti/Reference/Help center` | `tahti-web/HelpView.stories.tsx` |
| `Tahti/History/HistoryListSection` | `tahti-web/HistoryListSection.stories.tsx` |
| `Tahti/History/HistoryStatsSection` | `tahti-web/HistoryStatsSection.stories.tsx` |
| `Studio session` | `tahti-web/ImageLightbox.stories.tsx` |
| `Tahti/Media/ImageUploadField` | `tahti-web/ImageUploadField.stories.tsx` |
| `Tahti/Page/InPageNav` | `tahti-web/InPageNav.stories.tsx` |
| `Tahti/Reference/Latest view surfaces` | `tahti-web/LatestViewSurfaces.stories.tsx` |
| `Tahti/Widgets/LegalDocShell` | `tahti-web/LegalDocShell.stories.tsx` |
| `Tahti/Widgets/ListenAddonsPanel` | `tahti-web/ListenAddonsPanel.stories.tsx` |
| `Tahti/Widgets/ListenWidgetStoreDialog` | `tahti-web/ListenWidgetStoreDialog.stories.tsx` |
| `Tahti/Widgets/ListenerWidgetEmbed` | `tahti-web/ListenerWidgetEmbed.stories.tsx` |
| `Tahti/Widgets/ListenerWidgetsSection` | `tahti-web/ListenerWidgetsSection.stories.tsx` |
| `Tahti/Widgets/ListenerWorldMap` | `tahti-web/ListenerWorldMap.stories.tsx` |
| `Tahti/Widgets/MapCommentForm` | `tahti-web/MapCommentForm.stories.tsx` |
| `Tahti/Media/MediaIconActions` | `tahti-web/MediaIconActions.stories.tsx` |
| `Tahti/Community/MentionTextarea` | `tahti-web/MentionTextarea.stories.tsx` |
| `Tahti/Widgets/MermaidDiagram` | `tahti-web/MermaidDiagram.stories.tsx` |
| `Tahti/Chrome/MobileChrome` | `tahti-web/MobileChrome.stories.tsx` |
| `Tahti/Broadcast/MulticastConfigureDialog` | `tahti-web/MulticastConfigureDialog.stories.tsx` |
| `Tahti/Broadcast/MulticastSection` | `tahti-web/MulticastSection.stories.tsx` |
| `Northern Lights` | `tahti-web/MultitrackTimeline.stories.tsx` |
| `Tahti/Misc/NewsletterSubscribeToggle` | `tahti-web/NewsletterSubscribeToggle.stories.tsx` |
| `Tahti/Misc/NotificationToasts` | `tahti-web/NotificationToasts.stories.tsx` |
| `Tahti/Page/PageHeader` | `tahti-web/PageHeader.stories.tsx` |
| `Tahti/Page/PageStates` | `tahti-web/PageStates.stories.tsx` |
| `Tahti/Widgets/PageTourSpotlight` | `tahti-web/PageTourSpotlight.stories.tsx` |
| `Tahti/Studio/PinnedAnnouncementsPanel` | `tahti-web/PinnedAnnouncementsPanel.stories.tsx` |
| `Tahti/Track/PlayableTrackContextMenu` | `tahti-web/PlayableTrackContextMenu.stories.tsx` |
| `Tahti/Track/PlayableTrackTable` | `tahti-web/PlayableTrackTable.stories.tsx` |
| `Tahti/Widgets/PluginStorePanel` | `tahti-web/PluginStorePanel.stories.tsx` |
| `Tahti/Widgets/PluginStorePanel/Shared cards` | `tahti-web/PluginStoreShared.stories.tsx` |
| `Tahti/Misc/PortInventoryPanel` | `tahti-web/PortInventoryPanel.stories.tsx` |
| `Tahti/Studio/RadioBookingCalendar` | `tahti-web/RadioBookingCalendar.stories.tsx` |
| `Tahti/Settings/RadioBrowserDirectory` | `tahti-web/RadioBrowserDirectory.stories.tsx` |
| `Tahti/Widgets/PluginStorePanel/Radio category` | `tahti-web/RadioCategory.stories.tsx` |
| `Tahti/Widgets/RadioStationCover` | `tahti-web/RadioStationCover.stories.tsx` |
| `Tahti/Admin/RadioStationSuggestionsTab` | `tahti-web/RadioStationSuggestionsTab.stories.tsx` |
| `Tahti/Track/ReleaseTracklistDialog` | `tahti-web/ReleaseTracklistDialog.stories.tsx` |
| `Tahti/Settings/ReleaseVisualDefaultsPanel` | `tahti-web/ReleaseVisualDefaultsPanel.stories.tsx` |
| `Tahti/Studio/ReleasesPanel` | `tahti-web/ReleasesPanel.stories.tsx` |
| `Tahti/Misc/RightRailPanel` | `tahti-web/RightRailPanel.stories.tsx` |
| `Tahti/Media/RoundImageUploadButton` | `tahti-web/RoundImageUploadButton.stories.tsx` |
| `Tahti/Misc/ScreenAtlas` | `tahti-web/ScreenAtlas.stories.tsx` |
| `Tahti/Chrome/Section navigation` | `tahti-web/SectionNavigation.stories.tsx` |
| `Tahti/Navigation/SectionTabs` | `tahti-web/SectionTabs.stories.tsx` |
| `Tahti/Misc/SecurityTotpPanel` | `tahti-web/SecurityTotpPanel.stories.tsx` |
| `Tahti/Misc/SidebarBuildInfo` | `tahti-web/SidebarBuildInfo.stories.tsx` |
| `Tahti/Misc/SourceServiceIcon` | `tahti-web/SourceServiceIcon.stories.tsx` |
| `Tahti/Studio/StashFilesPanel` | `tahti-web/StashFilesPanel.stories.tsx` |
| `Tahti/Player/StemPlayer` | `tahti-web/StemPlayer.stories.tsx` |
| `Tahti/Studio/StreamManagerPanel` | `tahti-web/StreamManagerPanel.stories.tsx` |
| `Tahti/Studio/StudioGate` | `tahti-web/StudioGate.stories.tsx` |
| `Tahti/Studio/StudioGoLiveView` | `tahti-web/StudioGoLiveView.stories.tsx` |
| `Tahti/Studio/StudioModerationView` | `tahti-web/StudioModerationView.stories.tsx` |
| `Tahti/Studio/StudioNav` | `tahti-web/StudioNav.stories.tsx` |
| `Tahti/Page/StudioPanel` | `tahti-web/StudioPanel.stories.tsx` |
| `Tahti/Studio/StudioSoundsView` | `tahti-web/StudioSoundsView.stories.tsx` |
| `Tahti/Studio/StudioStripeView` | `tahti-web/StudioStripeView.stories.tsx` |
| `Tahti/Misc/SupportContactForm` | `tahti-web/SupportContactForm.stories.tsx` |
| `Tahti/Misc/TahtiLogo` | `tahti-web/TahtiLogo.stories.tsx` |
| `Tahti/Studio/TahtiRotationPlaylistEditor` | `tahti-web/TahtiRotationPlaylistEditor.stories.tsx` |
| `Tahti/Settings/ThemeEditor` | `tahti-web/ThemeEditor.stories.tsx` |
| `Tahti/Settings/ThemeVisualizationSettings` | `tahti-web/ThemeVisualizationSettings.stories.tsx` |
| `Tahti/Track/TimelineReactionBar` | `tahti-web/TimelineReactionBar.stories.tsx` |
| `Tahti/Track/TrackEditDialog` | `tahti-web/TrackEditDialog.stories.tsx` |
| `Tahti/Track/TrackExportPanel` | `tahti-web/TrackExportPanel.stories.tsx` |
| `Tahti/Track/TrackInfoDialog` | `tahti-web/TrackInfoDialog.stories.tsx` |
| `Tahti/Studio/TrackInsightsPanel` | `tahti-web/TrackInsightsPanel.latest.stories.tsx` |
| `Intro — Northern Lights` | `tahti-web/TracklistEditor.stories.tsx` |
| `Tahti/Track/UploadTrackDialog` | `tahti-web/UploadTrackDialog.stories.tsx` |
| `Tahti/Player/WaveformCanvas` | `tahti-web/WaveformCanvas.stories.tsx` |
| `Tahti/Widgets/WaveformMinimap` | `tahti-web/WaveformMinimap.stories.tsx` |
| `Tahti/Player/WaveformSeekbar` | `tahti-web/WaveformSeekbar.stories.tsx` |
| `Tahti/Discover/WidgetCard` | `tahti-web/WidgetCard.stories.tsx` |
| `Tahti/Discover/WidgetTrackRow` | `tahti-web/WidgetTrackRow.stories.tsx` |
| `Components/Alert` | `Alert.stories.tsx` |
| `Components/Badge` | `Badge.stories.tsx` |
| `Layout/BottomBar` | `BottomBar.stories.tsx` |
| `Components/Box` | `Box.stories.tsx` |
| `Components/Button` | `Button.stories.tsx` |
| `Components/HistoryCharts/CalendarHeatmap` | `CalendarHeatmap.stories.tsx` |
| `Components/Card` | `Card.stories.tsx` |
| `Layout/CardGrid` | `CardGrid.stories.tsx` |
| `Components/CardsRow` | `CardsRow.stories.tsx` |
| `Components/Combobox` | `Combobox.stories.tsx` |
| `Components/CopyButton` | `CopyButton.stories.tsx` |
| `Components/HistoryCharts/DayOfWeekChart` | `DayOfWeekChart.stories.tsx` |
| `Components/ActionSheet` | `ActionSheet.stories.tsx` |
| `Components/Dialog` | `Dialog.stories.tsx` |
| `Components/EmptyState` | `EmptyState.stories.tsx` |
| `Components/FavoriteButton` | `FavoriteButton.stories.tsx` |
| `Components/FilePicker` | `FilePicker.stories.tsx` |
| `Components/FilterChips` | `FilterChips.stories.tsx` |
| `Components/HistoryDayGroup` | `HistoryDayGroup.stories.tsx` |
| `Components/HistoryRow` | `HistoryRow.stories.tsx` |
| `Components/ImageReveal` | `ImageReveal.stories.tsx` |
| `Components/Input` | `Input.stories.tsx` |
| `Components/KeyCombo` | `KeyCombo.stories.tsx` |
| `Components/HistoryCharts/ListeningClock` | `ListeningClock.stories.tsx` |
| `Components/Loader` | `Loader.stories.tsx` |
| `Components/LogEntry` | `LogEntry.stories.tsx` |
| `Components/LogViewer` | `LogViewer.stories.tsx` |
| `Components/MediaArtwork` | `MediaArtwork.stories.tsx` |
| `Components/Mosaic` | `Mosaic.stories.tsx` |
| `Components/NewsWidget` | `NewsWidget.stories.tsx` |
| `Components/Pagination` | `Pagination.stories.tsx` |
| `Layout/PlayerBar` | `PlayerBar.stories.tsx` |
| `Layout/PlayerWorkspace` | `PlayerWorkspace.stories.tsx` |
| `Components/PluginItem` | `PluginItem.stories.tsx` |
| `Components/PluginStoreItem` | `PluginStoreItem.stories.tsx` |
| `Components/Popover` | `Popover.stories.tsx` |
| `Components/PulsingText` | `PulsingText.stories.tsx` |
| `Components/QueueItem` | `QueueItem.stories.tsx` |
| `Components/QueueItemPopover` | `QueueItemPopover.stories.tsx` |
| `Components/QueuePanel` | `QueuePanel.stories.tsx` |
| `Components/SaveButton` | `SaveButton.stories.tsx` |
| `Components/ScrollableArea` | `ScrollableArea.stories.tsx` |
| `Components/Select` | `Select.stories.tsx` |
| `Components/SettingsPanel` | `SettingsPanel.stories.tsx` |
| `Navigation/SidebarNavigation` | `SidebarNavigation.stories.tsx` |
| `Components/Slider` | `Slider.stories.tsx` |
| `Components/StatChip` | `StatChip.stories.tsx` |
| `Layout/Tabs` | `Tabs.stories.tsx` |
| `Remote/TahtiJam` | `TahtiJam.stories.tsx` |
| `Remote/TahtiJam/Connecting` | `TahtiJamConnecting.stories.tsx` |
| `Remote/TahtiJam/Controls` | `TahtiJamControls.stories.tsx` |
| `Remote/TahtiJam/Error` | `TahtiJamError.stories.tsx` |
| `Remote/TahtiJam/Header` | `TahtiJamHeader.stories.tsx` |
| `Remote/TahtiJam/NowPlaying` | `TahtiJamNowPlaying.stories.tsx` |
| `Remote/TahtiJam/Queue` | `TahtiJamQueue.stories.tsx` |
| `Remote/TahtiJam/SearchBar` | `TahtiJamSearchBar.stories.tsx` |
| `Queue is empty` | `TahtiJamSearchDrawer.stories.tsx` |
| `Some Bootleg Recording` | `TahtiJamSearchResultTrack.stories.tsx` |
| `Components/TahtiLogo` | `TahtiLogo.stories.tsx` |
| `Components/Textarea` | `Textarea.stories.tsx` |
| `Components/ThemeController` | `ThemeController.stories.tsx` |
| `Components/ThemeStoreItem` | `ThemeStoreItem.stories.tsx` |
| `Components/TitleBar` | `TitleBar.stories.tsx` |
| `Components/Toaster` | `Toaster.stories.tsx` |
| `Components/Toggle` | `Toggle.stories.tsx` |
| `Components/Tooltip` | `Tooltip.stories.tsx` |
| `Layout/TopBar` | `TopBar.stories.tsx` |
| `Components/TopList` | `TopList.stories.tsx` |
| `Components/TrackContextMenu` | `TrackContextMenu.stories.tsx` |
| `Title` | `TrackTable.stories.tsx` |
| `Components/ViewShell` | `ViewShell.stories.tsx` |
| `Welcome` | `Welcome.stories.tsx` |

If nothing matches: add a story with today’s states, flag `Missing states:` / `Orphan:`.

Rules: root `AGENTS.md` / `packages/tahti-web/AGENTS.md`.
