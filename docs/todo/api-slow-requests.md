...
- Why Redis stalled: host `vimage` device `sdd` (mounted at `/opt`, a spinning disk judging by ~95% busy at only ~13 MB/s) went from ~140 to ~1,240 writes/s. Utilisation was 0.95, iowait up to 78%, load1 60 (baseline ~3). Redis runs `--appendonly yes` (`infra/docker-compose.stack.yml:176-189`), so its AOF writes/fsyncs block on the saturated disk.
...
**Why a disk stall turns into a 60s request:** `apps/api/src/lib/redis.ts` creates the node-redis client with no command timeout, and the offline queue is on by default. `json-cache.ts` only falls back to `compute()` when Redis *errors*, not when it's slow. So every cached route (`/channels`, `/news`, `/addons/homepage`, `/stats`) waits on Redis for as long as the disk stays stuck.
...
- [ ] User: review/merge the PR, then deploy the API and alert rules (`ops/monitoring/vimage6/deploy.sh`).
- [ ] Host check on vimage (SSH was blocked from the agent session): where `stack_redis`, MinIO and Postgres volumes live, and whether `sdd` is the HDD. Then move Redis (ideally all three) to SSD, or set `no-appendfsync-on-rewrite yes`. The planned MinIO move to tahti.local (`../tahti-org/docs/todo/tahti-local-onboarding.md`) would also take MinIO's writes off this disk.
- [ ] If it recurs: `mc admin trace` on vimage during the episode to name the MinIO writer; consider shipping MinIO logs to Loki.
- [ ] Short-TTL cache around `runDependencyChecks` (lower priority).
- [ ] Prisma slow-query logging (`$on('query', ...)`, >200ms).
