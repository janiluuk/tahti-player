...
| ~~Tests~~ done (not compiled locally) | `local_library/tests.rs` | 1990 → `test_support.rs` (shared pool/WAV/seed helpers, 148) + `import_tests` 256, `tracks_tests` 243, `roots_tests` 240, `query_tests` 499, `playlists_tests` 240, `m3u_tests` 236, `reconcile_tests` 71, `open_tests` 55 (2026-09-25). Test bodies moved verbatim, 87 tests before and after; the other `*_tests.rs` and `organize.rs` now import helpers from `test_support`. **Not compiled yet:** no Rust toolchain on this machine, imports checked by script only - run `cargo test --lib` before merging. | Complete once `cargo test --lib` passes. |
...
- **Open:** not compiled. There is no `cargo` on this machine (and about 11 GB free disk), so `cargo test --lib` in `packages/player/src-tauri` still needs to run once. CI only builds the Tauri app, it does not run the lib tests.
