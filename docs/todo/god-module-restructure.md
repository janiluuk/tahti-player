...
| ~~Tests~~ done (not compiled locally) | `local_library/tests.rs` | 1990 → `test_support.rs` (shared pool/WAV/seed helpers, 148) + `import_tests` 256, `tracks_tests` 243, `roots_tests` 240, `query_tests` 499, `playlists_tests` 240, `m3u_tests` 236, `reconcile_tests` 71, `open_tests` 55 (2026-09-25). Test bodies moved verbatim, 87 tests before and after; the other `*_tests.rs` and `organize.rs` now import helpers from `test_support`. Compiled and run 2026-09-25 (rustup installed): the split had dropped `const ALL_SORTS`, now restored in `query_tests.rs`. `cargo test --lib`: 215 passed, 2 failed, identical to `origin/master` on the same Mac (see fifteenth pass). | Complete. |
...
### Fifteenth pass (2026-09-25): split `local_library/tests.rs`

- `tests.rs` (1990 lines, 87 tests) split into one sibling test file per module, matching the existing `catalog_tests.rs` / `backup_tests.rs` layout: `import_tests`, `tracks_tests`, `roots_tests`, `query_tests`, `playlists_tests`, `m3u_tests`, `reconcile_tests`, `open_tests`. Shared helpers (`pool`, `write_wav`, `write_wav_tagged`, `seed_generated_rows`, `seed_varied_rows`, `import_into_root`, `entry_titles`) moved to `test_support.rs`.
- Test bodies unchanged; each file imports only what it uses.
- Compiled and run 2026-09-25 after installing rustup. The first compile failed: the split script had dropped `const ALL_SORTS` (it only matched `fn`/`static`), now restored in `query_tests.rs`. The old `// ---` section banners were dropped on purpose; each section is its own file now.
- `cargo test --lib`: 215 passed, 2 failed, 5 ignored - the same on the branch and on `origin/master`. The two failures are pre-existing on macOS and unrelated to the split: `backup_tests::backup_restores_into_a_fresh_profile_with_remapped_roots` (found/missing counts swapped after remapping) and `m3u_tests::import_keeps_unresolved_entries_and_relinks_them_when_the_file_arrives` ("relinked by path"). Both compare paths under the temp dir, where macOS's `/var` is a symlink to `/private/var`. Not investigated further.
- CI only builds the Tauri app; it does not run the lib tests, so these failures would not show up there either way.

### Sixteenth pass (2026-09-25): `<Link><Button/></Link>` nesting removed

- New in `@tahti-player/ui`: `ButtonLink`, a router `Link` via `createLink` that keeps route typing, and `ButtonAnchor`, a plain `<a>` for external URLs and downloads. Both are a single anchor styled with the exported `buttonVariants`, with a `focus-visible` ring. `disabled` drops the `href` and sets `aria-disabled`. Includes a Storybook story (`ButtonLink.stories.tsx`) and 4 tests.
- All 92 nested sites in tahti-web (82 `Link`, 10 `<a>`, 59 files) were converted with an AST codemod. `className`s were merged, `type="button"` dropped, and the Button's `disabled`/`onClick` moved onto the link. The old markup gave two tab stops per action and a button inside an anchor; a disabled Button inside a Link could still be followed by keyboard.
- Layout: the anchor is now the flex item itself, so 15 sites directly in a stretching `flex-col` got `w-fit` to keep their natural width. `StudioStripeView`'s wrapper `inline-block` was dropped because it would have overridden `inline-flex`.
- tahti-web: 921 tests passing, `tsc`/`eslint` clean. ui: 358 passing. Checked in the running app (mock mode) on Discover's artist-pick card only; the signed-in studio/admin pages were not checked visually.
