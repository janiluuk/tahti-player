...
| --- | --- | --- | --- |
| P0 | `packages/player/src-tauri/src/local_library/mod.rs` | 1693 | Types (`LibraryTrack`, filters, sort), query builder (`where_clause`/`order_clause`/`ListQuery`), import pipeline (batch, progress, walk), roots (scan/relink), facets/totals, Tauri command wrappers, `LibraryState`/pool/migrations. Target: `mod.rs` = re-exports + state only. |
| P0 | `packages/player/src-tauri/src/local_library/catalog.rs` | 1222 | edits+undo, user data (rating/color/tags), play log, duplicates+hashing, merge, commands. |
...
1. [ ] `local_library/mod.rs` first: split into submodules per the seams above, no behavior change. Do it as its own commit, **between** feature phases (not mixed with Phase 5 work, since other sessions touch the same files).
2. [ ] `catalog.rs`, then `DesktopLibraryPanel.tsx` and `nativeLibrary.ts`.
