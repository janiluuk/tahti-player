...
- [x] Collection editor: details and backdrop gallery now save in one request (2026-09-23, tahti-org #557: `PATCH /api/me/collections/:slug` takes `gallery`, applied in the same row update). The same PR found `visibility` was stripped by the API schema, so "Unlisted"/"Private" never saved; it is accepted now (editor Private ↔ API `DRAFT`). **Decision needed:** the editor also sends `releaseDate`, `genres` and `backdropUrl`, which collections have no columns for, so they are still dropped. Add columns (migration) or remove them from the editor.
