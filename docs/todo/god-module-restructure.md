...
- [ ] `showId` on `UpcomingBroadcast` (Schedule links upcoming items to shows by title today). Investigated, not implemented this pass: `RadioSlotBooking` has no direct FK to `LiveShowSeries` — the only link is indirect through an optional `LiveShowEpisode` (cardinality not constrained to 1). Closing this needs a design decision (join through the optional episode vs. add a real FK column + migration), not just a field add — moderate effort, deferred to a pass with that decision made.
- [ ] Top-list windows for "Custom" and "1 day" ranges (Stats falls back to 30 days). Investigated, not implemented this pass: `top-tracks`/`top-countries`/`top-lists` only accept enum ranges (`1|7|30|all` / `week|month|half_year|all_time`), no `from`/`to`. The plain plays/summary endpoint (`GET /api/me/stats`) already has the exact pattern needed (`buildArtistPlaysStats` branches on `customFrom`/`customTo`) — extending the three top-list routes to match means touching 3 route handlers + `artist-stats.ts` + `top-lists.ts` + shared DTO schemas. Moderate effort, following an established pattern; deferred to its own pass rather than rushed alongside the other three.
...
**Splits still to do** (baselined over 800 lines): `StudioHomeView` 562 and `StudioSoundView` 533/`StudioStatsView` 462/`StudioScheduleView` 137/`StudioGoLiveView` 798+238 hook — all now under or near baseline, see 2026-09-22 entry below. `TrackEditDialog`/`LocalPlaylists`/`StreamManagerPanel` done 2026-09-23 (see sweep table). Still open: `mcp/metadata.rs` (Rust, not part of this pass) and the 11 `requestJson` near-copies that differ (see api splits). `StudioGoLiveView` itself is still 798 lines (JSX-heavy; state/effects now live in `go-live/useGoLiveState.ts`) — a further split of its JSX into panels is still open if it grows more.
...
- [ ] Collection editor: the details save is still two requests (a partial failure is now reported, not rolled back). Not fixable purely client-side without a combined backend endpoint (details + gallery are two separate API resources) — tracked as a backend need, not a frontend bug.
