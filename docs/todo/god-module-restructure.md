...
**Tests missing** (only `trimToCuts` plus the plugin-store and api tests exist): the studio fixes above — collection editor add-track keeps form edits, playlists confirm-before-remove, Branding replace-upload order, Updates confirm-before-send, Distribution confirm-before-submit, Stats top-list-only refetch, Schedule disabled create-only fields, Go live confirm.
- [x] 2026-09-22: show detail uploads a picked image before saving (`useShowDetail.test.ts` — a picked thumbnail/backdrop file is uploaded and the *uploaded* URL, never the local `blob:` preview, is what gets saved; a failed upload never calls `patchShowSeries`).
