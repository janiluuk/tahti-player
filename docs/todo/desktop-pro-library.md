...
- [ ] Add catalog schema/migrations and transactional repository; define stable IDs, file-location identity, metadata precedence, missing-file states and migration/backup policy. — Stable IDs (UUID v4) and file-location identity (path is the unique key; see new `same_filename_in_different_folders_are_distinct_tracks` test) already exist from the 2026-09-10 slice. Metadata precedence (tags override filename fallback, never invent an artist) is implemented in `metadata.rs` and now has explicit test coverage (`imports_untagged_file_falling_back_to_filename`). **Not done:** an actual missing-file *state* column (today `resolve_path` only reports missing transiently, per-call) and a written migration/backup policy — deliberately left to Phase 1, which already owns "mark missing/unavailable files" as its own bullet; adding a schema column now risked pre-empting that design decision without a native toolchain available to verify the migration runs.
...
**Verification note:** this environment has no Rust toolchain (`cargo`/`rustc` not on PATH), so the `tests.rs` additions above were written and manually re-verified by hand (including catching and fixing one arithmetic bug in a search-count assertion) but **could not be compiled or run**. `AppShell.tsx`'s change was verified for real: `eslint` clean, and `tsc --noEmit` shows the same pre-existing unrelated route-typing errors with or without the change (confirmed by diffing tsc output with the edit stashed out). `pnpm --filter @tahti-player/tahti-web test` in this fresh worktree has 25 unrelated failing test files due to a jsdom/localStorage environment issue (`--localstorage-file was not provided`) present before this change and unrelated to anything touched here — not a regression introduced by this slice.

**Exit demo:** signed-out desktop opens Local files offline (fixed this pass); catalog migrations survive restart (pre-existing); a typed query returns a fixture page (pre-existing + new fixtures); browser does not invoke native commands (pre-existing, confirmed by design).
