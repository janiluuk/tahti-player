...
19. **`useChannelLayoutEditing` hook extraction** — **2026-09-16:**
    `ChannelView.tsx`'s page-layout editing state (the `layout` array,
    `activePresetId`, `editing`/`selectedId`/`dragId`/`moveDrag`/
    `layoutDirty`, and the `updateLayout`/`removeLayoutItem`/`saveLayout`
    helpers + the slug-change resync effect) moved verbatim into a new
    `hooks/useChannelLayoutEditing.ts`. Deliberately **not** a narrowing
    refactor — the hook exports every piece of state and its setter
    rather than hiding them behind a smaller API, since (per item 14's
    original finding) the rest of `ChannelView.tsx` still reads
    `editing`/`layout` directly in a dozen places and callers still
    compose look-editing/preset side effects (`applyPreset`, `startEdit`,
    etc.) themselves using the setters. Pure state relocation, zero
    behavior change — confirmed by diff-reading every moved block
    (nothing rewritten, only relocated + destructured back identically
    at the call site) rather than assuming. Cleaned up 4 now-unused
    imports (`loadChannelPageLayout`/`loadChannelLayoutPresetId`/
    `saveChannelPageLayout`/`saveChannelLayoutPresetId`) left behind in
    `ChannelView.tsx` once their only call sites moved into the hook.
    No new tests added (neither file had any before this pass either —
    risk was managed by keeping the move byte-for-byte mechanical and
    verifying via the full existing suite, not by adding new coverage).
